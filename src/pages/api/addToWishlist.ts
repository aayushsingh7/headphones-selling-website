...
const addToWishlist = async (req:NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    let addProduct = await mongoClient
      .db()
      .collection("user")
      .updateOne({ _id: new ObjectId(userId) }, { $push: { wishlist: data } });

    if (addProduct.modifiedCount === 1) {
      res.status(200).send({ success: true, message: "Item added to wishlist" });
    } else {
      res.status(400).send({
        success: false,
